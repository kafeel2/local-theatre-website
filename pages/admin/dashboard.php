...
<div class="flex min-h-screen">
  <!-- Sidebar -->
  <div class="hidden md:flex flex-col w-64 [#8B0000]">
    ...
    <div class="flex flex-col flex-1 overflow-y-auto">
      <nav class="flex-1 px-2 py-4 bg-[#8B0000]">
        <?php if ($_SESSION['role'] === 'admin'): ?>
          <a href="<?= ROOT_DIR ?>admin/dashboard" class="flex items-center px-4 py-2 text-gray-100 hover:bg-gray-700">
            Dashboard
          </a>
        <?php endif; ?>

        <a href="<?= ROOT_DIR ?>admin/comments" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-700">
          Manage Comments
        </a>

        <a href="<?= ROOT_DIR ?>admin/manage-blogs" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-700">
          Manage Blogs
        </a>
        ...
        <a href="<?= ROOT_DIR ?>admin/messages" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-700">
          Messages
        </a>

        <a href="<?= ROOT_DIR ?>admin/settings" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-700">
          Settings
        </a>
      </nav>
    </div>
    ...
    <table class="min-w-full bg-white">
      <thead class="bg-gray-100 whitespace-nowrap">
        <tr>
          ...
          <th class="p-4 text-left text-[13px] font-semibold text-slate-900">Joined At</th>
          <th class="p-4 text-left text-[13px] font-semibold text-slate-900">Total Comments</th>
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="p-4">
              <div class="flex items-center space-x-2">
                <button onclick="window.location.href='edit-user?uid=<?= urlencode($uid) ?>'" title="Edit">Edit User</button>
                <button class="modal-show" data-user-id="<?= $uid ?>" title="Delete">Delete User</button>
              </div>
            </td>
          </tr>
        <?php endwhile ?>
