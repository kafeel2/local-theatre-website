      ...
      <tbody class="whitespace-nowrap">
        <?php while ($blogs->fetch()) : ?>
          <tr class="hover:bg-gray-50">
            <td class="p-4 text-[15px] text-slate-900 font-medium"><?= htmlspecialchars($bid) ?></td>
            ...
            <td class="p-4 text-[15px] text-slate-600 font-medium"><?= htmlspecialchars($created) ?></td>
            <td class="p-4 text-[15px] text-slate-600 font-medium"><?= htmlspecialchars($status) ?></td>
            <td class="p-4">
              <div class="flex items-center space-x-2">
                <button type="button"
                  onclick="window.location.href='edit-blog?bid=<?= urlencode($bid) ?>'"
                  class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline"
                  title="Edit">
                  Edit
                </button>
                <button class="modal-show" data-user-id="<?= $bid ?>" title="Delete">Delete</button>
              </div>
            </td>
          </tr>
        <?php endwhile ?>
      ...
      <div class="w-full max-w-lg bg-white shadow-lg rounded-lg p-6 relative">
        ...
        <div class="my-6">
          <p class="text-gray-600 text-sm leading-relaxed">
            Are you sure you want to delete the blog with ID <span id="modal_user_id"></span>? This will also remove all related comments.
          </p>
        </div>
...
<script>
  document.querySelectorAll('.modal-show').forEach(button => {
    button.addEventListener("click", function () {
      ...
      document.getElementById('modal_user_id').textContent = blogId;
